...
pub fn openid4vp_main(credman: &mut impl CredmanApi) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if matcher_data_buffer.len() < 4 {
        log::error!("Matcher data buffer is too small: {}", matcher_data_buffer.len());
    ...
    let registry: Registry = match DeJson::deserialize_json(matcher_data_str) {
        Ok(data) => data,
        Err(e) => {
            log::error!("Failed to deserialize registry: {:?}. JSON snippet: {}", e, &matcher_data_str[..std::cmp::min(100, matcher_data_str.len())]);
            return Err(e.into());
        }
    };
    ...
    let request: OpenId4VpRequest = match DeJson::deserialize_json(request_str) {
        Ok(req) => req,
        Err(e) => {
            log::error!("Failed to deserialize request: {:?}. JSON: {}", e, request_str);
            return Err(e.into());
        }
    };
...
fn serialize_json_value(val: &JsonValue) -> String {
    match val {
        ...
        JsonValue::Object(obj) => {
            let items: Vec<String> = obj.iter().map(|(k, v)| format!("\"{}\":{}", k, serialize_json_value(v))).collect();
            format!("{{{}}}", items.join(","))
        }
    }
}
...
fn report_match_result(
    ...
    if !res.matched_credential_sets.is_empty() {
        ...
        for opt in first_set {
            let set_id_str = if !opt.set_id.is_empty() {
                format!("req:{};set:{};option:{}", request_idx, opt.set_id, opt.option_id)
            } else {
            ...
            if wasm_version > 1 {
                ...
                report_credential_set_length(credman, &set_id_cstr, opt.matched_credential_ids.len(), 1, &res.matched_credential_sets);
            }
            ...
            for cred_id in &opt.matched_credential_ids {
                if let Some(doc) = res.matched_credentials.get(cred_id) {
                    report_matched_credential(
                        ...
                        request_idx,
                        &set_id_cstr,
                        if opt.set_id.is_empty() { None } else { Some(&opt.set_id) },
                        if opt.option_id.is_empty() { None } else { Some(&opt.option_id) },
                        creds_blob,
                        &transaction_info,
                    )?;
                    doc_idx += 1;
    ...
    if let Some(inline) = &res.inline_issuance {
        ...
        let title_cstr = inline.title.as_ref().map(|s| CString::new(s.clone())).transpose()?;
        let subtitle_cstr = inline.subtitle.as_ref().map(|s| CString::new(s.clone())).transpose()?;
        let icon_bytes = inline.icon.as_ref().map(|i| &creds_blob[i.start..i.start + i.length]);

        credman.add_inline_issuance_entry(&cred_id_cstr, icon_bytes, title_cstr.as_deref(), subtitle_cstr.as_deref());
    }
...
mod test {
    ...
    impl CredmanApi for FakeCredman {
        ...
        fn add_string_id_entry(&mut self, _id: &CStr, _icon: Option<&[u8]>, _title: Option<&CStr>, _subtitle: Option<&CStr>, _disclaimer: Option<&CStr>, _warning: Option<&CStr>) {}
        ...
        fn add_entry_to_set(&mut self, cred_id: &CStr, _icon: Option<&[u8]>, title: Option<&CStr>, subtitle: Option<&CStr>, disclaimer: Option<&CStr>, warning: Option<&CStr>, _metadata: Option<&CStr>, set_id: &CStr, set_index: i32) {
            ...
            let entry = FakeEntry {
                ...
                title: title.map(|s| s.to_str().unwrap().to_string()).unwrap_or_default(),
                subtitle: subtitle.map(|s| s.to_str().unwrap().to_string()).unwrap_or_default(),
                disclaimer: disclaimer.map(|s| s.to_str().unwrap().to_string()).unwrap_or_default(),
                warning: warning.map(|s| s.to_str().unwrap().to_string()).unwrap_or_default(),
        ...
        fn add_field_to_entry_set(&mut self, cred_id: &CStr, field_display_name: &CStr, field_display_value: Option<&CStr>, set_id: &CStr, set_index: i32) {
            ...
            let f_val = field_display_value.map(|s| s.to_str().unwrap().to_string()).unwrap_or_default();
            self.entry_sets.get_mut(&s_id).unwrap().entries.get_mut(&set_index.to_string()).unwrap().get_mut(&c_id).unwrap().fields.push((f_name, f_val));
        }
        fn add_payment_entry_to_set_v2(&mut self, cred_id: &CStr, merchant_name: Option<&CStr>, _method_name: Option<&CStr>, _method_subtitle: Option<&CStr>, _icon: Option<&[u8]>, transaction_amount: Option<&CStr>, _bank_icon: Option<&[u8]>, _provider_icon: Option<&[u8]>, additional_info: Option<&CStr>, _metadata: Option<&CStr>, set_id: &CStr, set_index: i32) {
            let s_id = set_id.to_str().unwrap().to_string();
            let c_id = cred_id.to_str().unwrap().to_string();
            let entry = FakeEntry {
                cred_id: c_id.clone(),
                ...
                metadata_display_text: String::new(),
                fields: Vec::new(),
                merchant_name: merchant_name.map(|s| s.to_str().unwrap().to_string()).unwrap_or_default(),
        ...
        fn add_metadata_display_text_to_entry_set(&mut self, cred_id: &CStr, metadata_display_text: &CStr, set_id: &CStr, set_index: i32) {
            let s_id = set_id.to_str().unwrap().to_string();
            let c_id = cred_id.to_str().unwrap().to_string();
            self.entry_sets.get_mut(&s_id).unwrap().entries.get_mut(&set_index.to_string()).unwrap().get_mut(&c_id).unwrap().metadata_display_text = metadata_display_text.to_str().unwrap().to_string();
        }
    }
    ...
    fn normalize_json(val: JsonValue) -> JsonValue {
        match val {
            JsonValue::Array(arr) => {
                ...
                let is_map = !arr.is_empty() && arr.iter().all(|item| {
                    if let JsonValue::Array(pair) = item {
                        pair.len() == 2 && matches!(pair[0], JsonValue::Number(_)) && matches!(pair[1], JsonValue::Object(_))
                    } else {
                        false
                    }
                });
                ...
                if is_map {
                    ...
                    let mut normalized_arr: Vec<JsonValue> = arr.into_iter().map(normalize_json).collect();
    ...
    fn run_test_impl(test_name: &str, custom_registry: Option<&str>) {
        ...
        assert_eq!(result_normalized, expected_normalized, "Test {} failed", test_name);
    }
    ...
    define_test!(tc26_dcql_query_set_fail_required, "TC26_DcqlQuerySetFailRequired");
    define_test!(tc27_dcql_query_set_fail_optional, "TC27_DcqlQuerySetFailOptional");
    define_test!(tc28_dcql_query_complex_overlapping_sets, "TC28_DcqlQueryComplexOverlappingSets");
    define_test!(tc29_dcql_query_openid4vp_spec_example, "TC29_DcqlQueryOpenID4VPSpecExample");
    ...
    fn tc37_wasm_metadata_text() {
        let manifest_dir = std::env::var("CARGO_MANIFEST_DIR").unwrap();
        let testdata_dir = std::path::PathBuf::from(manifest_dir).join("../matcher/testdata");
        let mut registry_json = std::fs::read_to_string(testdata_dir.join("registry.json")).unwrap();
