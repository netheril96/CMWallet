...
pub trait CredmanApi {
    ...
    fn add_metadata_display_text_to_entry_set(
        &mut self,
        cred_id: &CStr,
        metadata_display_text: Option<&CStr>,
        set_id: &CStr,
        set_index: i32,
    );
}
...
impl CredmanApi for CredmanApiImpl {
    fn get_request_buffer(&self) -> Vec<u8> {
        let mut size: u32 = 0;
        unsafe {
            crate::bindings::GetRequestSize(&mut size);
        }
        log::debug!("Host requested request buffer of size: {}", size);
        let mut r = vec![0; size as usize];
        unsafe {
            crate::bindings::GetRequestBuffer(r.as_mut_ptr() as *mut c_void);
        }
        r
    }
    fn get_registered_data(&self) -> Vec<u8> {
        let mut size: u32 = 0;
        unsafe {
            crate::bindings::GetCredentialsSize(&mut size);
        }
        log::debug!("Host requested registered data buffer of size: {}", size);
        let mut r = vec![0; size.try_into().unwrap()];
        unsafe {
            crate::bindings::ReadCredentialsBuffer(r.as_mut_ptr() as *mut c_void, 0, size as usize);
        }
        r
    }
    fn add_string_id_entry(
        ...
        unsafe {
            crate::bindings::AddStringIdEntry(
                entry_id.as_ptr(),
                icon_bytes,
                icon_length,
    ...
    fn add_entry_set(&mut self, set_id: &CStr, set_length: i32) {
        unsafe {
            crate::bindings::AddEntrySet(set_id.as_ptr(), set_length);
        }
    }
    ...
    fn add_field_to_entry_set(
        ...
        unsafe {
            crate::bindings::AddFieldToEntrySet(
                cred_id.as_ptr(),
    ...
    fn add_payment_entry_to_set_v2(
        ...
        let payment_method_icon_bytes =
            payment_method_icon.map_or(std::ptr::null(), |x| x.as_ptr()) as *const i8;
        let payment_method_icon_length = payment_method_icon.map_or(0, |x| x.len());
        let bank_icon_bytes = bank_icon.map_or(std::ptr::null(), |x| x.as_ptr()) as *const i8;
        let bank_icon_length = bank_icon.map_or(0, |x| x.len());
        let payment_provider_icon_bytes =
            payment_provider_icon.map_or(std::ptr::null(), |x| x.as_ptr()) as *const i8;
        let payment_provider_icon_length = payment_provider_icon.map_or(0, |x| x.len());

        unsafe {
            crate::bindings::AddPaymentEntryToSetV2(
                cred_id.as_ptr(),
                merchant_name.as_ptr(),
                payment_method_name.as_ptr(),
                payment_method_subtitle.map_or(std::ptr::null(), |x| x.as_ptr()),
                payment_method_icon_bytes,
                payment_method_icon_length,
                transaction_amount.map_or(std::ptr::null(), |x| x.as_ptr()),
                bank_icon_bytes,
                bank_icon_length,
                payment_provider_icon_bytes,
                payment_provider_icon_length,
    ...
    fn add_inline_issuance_entry(
        ...
        let icon_length = icon.map_or(0, |x| x.len());
        unsafe {
            crate::bindings::AddInlineIssuanceEntry(
                cred_id.as_ptr(),
                icon_bytes,
                icon_length,
                title.map_or(std::ptr::null(), |x| x.as_ptr()),
    ...
    fn get_wasm_version(&self) -> u32 {
        ...
        unsafe {
            crate::bindings::GetWasmVersion(&mut version);
        }
        version
    }
    ...
    fn add_metadata_display_text_to_entry_set(
        ...
        unsafe {
            crate::bindings::AddMetadataDisplayTextToEntrySet(
                cred_id.as_ptr(),
                metadata_display_text.map_or(std::ptr::null(), |x| x.as_ptr()),
                set_id.as_ptr(),
                set_index,
            );
        }
